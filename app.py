...
def extract_text_from_id(image_path):
    """Extract text from ID card using OCR with enhanced preprocessing."""
    try:
        # Read the image
        ...
        # Configure Tesseract parameters for better accuracy and speed
        ...
        for i, line in enumerate(lines):
            ...
            # Name patterns:
            # 1. At least 3 capital letters + space + at least 3 capital letters
            # 2. Any text without numbers where next line starts with B, M, or P
            if (re.match(r'^[A-Z]{3,}\s[A-Z]{3,}$', line) or 
                (not any(c.isdigit() for c in line) and 
                 i < len(lines) - 1 and 
                 re.sub(r'^[\s,]+', '', lines[i + 1]).startswith(('B', 'M', 'P')))) and not extracted_info['name']:
            ...
            if 'CSE' in line:
                extracted_info['branch'] = 'CSE'
            elif 'ECE' in line:
                extracted_info['branch'] = 'ECE'
            elif 'EEE' in line:
                extracted_info['branch'] = 'EEE'
            
            # Roll Number (AP + 11 digits)
            roll_match = re.search(r'AP\d{11}', line)
            if roll_match:
                extracted_info['roll_number'] = roll_match.group(0)
            
            # Year
            year_match = re.search(r'20\d{2}-20\d{2}', line)
            if year_match:
        ...
        extracted_info['debug'] = {
            ...
            'thresholding': 'Applied adaptive thresholding',
            'enhancement': 'Applied CLAHE enhancement',
            'preprocessing': 'Applied bilateral filtering and resizing'
        }
